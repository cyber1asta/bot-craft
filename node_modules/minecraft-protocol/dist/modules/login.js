...
module.exports = function (opts, client, server) {
  if (client.isServer) installServer(opts, client, server);else installClient(opts, client);
};

// Sets client.username, emits 'login' event on client when it is done.
function installServer(opts, client, server) {
  if (!server.serverKey) server.serverKey = ursa.generatePrivateKey(1024);

  // If client takes longer than 10 seconds to connect, kick him.
  var loginKickTimer = setTimeout(function () {
    return client.end('LoginTimeout');
  }, 10 * 1000);
  client.on('end', function () {
    return clearTimeout(loginKickTimer);
  });

  client.once("set_protocol", function (packet) {
    if (packet.nextState == 2) client.state = states.LOGIN;
  });

  client.once("login_start", function (packet) {
    client.username = packet.username;
    if (opts.onlineMode) loginOnlineMode(packet);else {
  ...
  function loginOnlineMode(packet) {
    ...
    client.once('encryption_begin', function (packet) {
      ...
      try {
        ...
        if (!bufferEqual(newVerifyToken, verifyToken)) return client.end('DidNotEncryptVerifyTokenProperly');
      ...
      validateSession(client.username, digest, function (err, uuid, profile) {
        if (err) return client.end("Failed to verify username!");
...
//# sourceMappingURL=../maps/modules/login.js.map