...
module.exports = function (opts, client, server) {
  if (!client.isServer) throw new Error("Client needs to be serverside");

  // First, inject the state-changing logic for the set_protocol packet
  client.once("set_protocol", function (packet) {
    if (packet.nextState == 1) client.state = states.STATUS;
  });
  ...
  client.once("ping_start", function () {
    ...
    if (opts.beforePing) response = opts.beforePing(response, client) || response;
...
//# sourceMappingURL=../maps/modules/ping.js.map