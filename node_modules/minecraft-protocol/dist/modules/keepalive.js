...
module.exports = function (opts, client) {
  if (client.isServer) installKeepaliveServer(opts, client);else installKeepaliveClient(opts, client);
};
...
function installKeepaliveServer(opts, client) {
  ...
  function keepAliveLoop() {
    ...
    if (elapsed > timeout) return client.end('KeepAliveTimeout');else {
      lastKeepAliveId = Math.floor(Math.random() * Math.pow(2, 31));
  ...
  if (client.state !== states.PLAY) {
    client.once('state', function (newstate) {
      if (newstate === states.PLAY) interval = setInterval(keepAliveLoop, checkInterval);
    });
  } else {
...
//# sourceMappingURL=../maps/modules/keepalive.js.map